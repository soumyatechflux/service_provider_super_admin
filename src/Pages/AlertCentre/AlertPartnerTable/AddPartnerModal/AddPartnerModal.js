...
const AddPartnerModal = ({ show, onClose, onSave,fetchAlertData}) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVICE_PROVIDER_SUPER_ADMIN_BASE_API_URL}/api/firebase/send_notification/partners`, {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        ...
        if (successfulMessages.length > 0) {
          ...
          fetchAlertData();
          onSave(formData);  // Pass the form data back to parent
