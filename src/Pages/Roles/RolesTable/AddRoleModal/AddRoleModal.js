...
const AddRoleModal = ({ show, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    const selectedPermissionIds = permissionsList
      .filter((permission) => formData.permissions[permission.permission_name]) // Check if the permission is selected
      .map((permission) => permission.permission_id); // Use permission_id for the payload
    ...
    const payload = {
      role: {
        ...
        permissions: selectedPermissionIds, // Array of permission IDs
      },
    };
  
    console.log("Payload being sent:", payload); // Log the payload to confirm its structure
    ...
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <div className="form-group">
          ...
          <div className="row">
            {[permissionsList.slice(0, Math.ceil(permissionsList.length / 2)),
            permissionsList.slice(Math.ceil(permissionsList.length / 2))].map(
              (column, colIdx) => (
                    ...
                    <div className="form-check" key={permission.id}>
                      <input
                        ...
                        id={permission.permission_name
                          ...
                          .replace(/\s+/g, "-")}
                        checked={
                          formData.permissions[permission.permission_name] ||
                          false
                        }
                        onChange={(e) =>
