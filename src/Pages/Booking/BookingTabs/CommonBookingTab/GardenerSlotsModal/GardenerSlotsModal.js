    ...
    <div className={`modal fade ${show ? 'show' : ''}`} style={{ display: show ? 'block' : 'none', backgroundColor: 'rgba(0,0,0,0.5)' }}>
      <div className="modal-dialog modal-dialog-centered modal-xl">
        <div className="modal-content">
          ...
          <div className="modal-body">
            {bookingCompleteDates && bookingCompleteDates.length > 0 ? (
              <div className="table-responsive">
                <table className="table table-bordered table-hover">
                  <thead className="thead-light">
                    <tr>
                  ...
                  <tbody>
                    {bookingCompleteDates.map((date, index) => (
                      <tr key={index}>
                        <td>{index + 1}</td>
                        <td>{formatDateTime(date.reached_location)}</td>
                        <td>{formatDateTime(date.start_job)}</td>
                        <td>{formatDateTime(date.end_job)}</td>
                        <td>{formatDateTime(date.payment)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="alert alert-info">
                No visit details available for this booking.
              </div>
            )}
          </div>
          <div className="modal-footer">
